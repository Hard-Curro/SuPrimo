/*
   * LOGIN: EL USUARIO QUE ENTRA POR LA API NO SE ALMACENA EN LOCALSTORAGE  
   * AREAPERSONAL: EL BOTON DE ELIMINAR NO FUNCIONA CON LOS DE LA API (eliminar por api)
   * LOS PRODUCTOS CREADOS NO TIENEN PAGIAN PROPIA  
   * NO PUEDE EXISTIR EL MISMO USUARIO ACTIVO Y ELIMINADO
   * SOLO 1 CUENTA POR CORREO
*/
...
loginForm.addEventListener('submit', async (e) => {
    ...
    const validUserDeleted = UsersDeletd.find(user => user.username === username && user.password === password);
...
function buscarUsuario(nombre) {
    fetch('https://fakestoreapi.com/users')
      ...
      .then(data => {
        const usuarioEncontrado = data.find(usuario => usuario.username === nombre);
